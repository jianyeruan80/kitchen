angular.module('kitchen.controllers', [])
    ...
    .controller('TabCtrl', function($scope, $rootScope, $timeout, $ionicModal, localStorageService, $http, $location, $filter,$translate,localStorageService) {
        ...
        $scope.audio = new Audio('img/2946.wav');
...
{key:'q','width':'keyAll'},{key:'w','width':'keyAll'},{key:'e','width':'keyAll'},{key:'r','width':'keyAll'},{key:'t','width':'keyAll'},{key:'y','width':'keyAll'},{key:'u','width':'keyAll'},{key:'i','width':'keyAll'},{key:'o','width':'keyAll'},{key:'p','width':'keyAll'},{key:'','width':'keyBorder'},{key:'1','width':'keyAll'},{key:'2','width':'keyAll'},{key:'3','width':'keyAll'},
{key:'a','width':'keyAll'},{key:'s','width':'keyAll'},{key:'d','width':'keyAll'},{key:'f','width':'keyAll'},{key:'g','width':'keyAll'},{key:'h','width':'keyAll'},{key:'j','width':'keyAll'},{key:'k','width':'keyAll'},{key:'l','width':'keyAll'},{key:'#','width':'keyAll'},{key:'','width':'keyBorder'},{key:'4','width':'keyAll'},{key:'5','width':'keyAll'},{key:'6','width':'keyAll'}
,{key:'z','width':'keyAll'},{key:'x','width':'keyAll'},{key:'c','width':'keyAll'},{key:'v','width':'keyAll'},{key:'b','width':'keyAll'},{key:'n','width':'keyAll'},{key:'m','width':'keyAll'},{key:' ','width':'keyAll'},{key:'Backspace','width':'Backspace'},{key:'','width':'keyBorder'},{key:'7','width':'keyAll'},{key:'8','width':'keyAll'},{key:'9','width':'keyAll'}
,{key:'','width':'Otherspace'},{key:'','width':'keyBorder'},{key:'*','width':'keyAll'},{key:'0','width':'keyAll'},{key:'#','width':'keyAll'}
                    ...
                    angular.forEach(value.orderItemList, function(orderValue, orderKey) {
                        angular.forEach(orderValue.orderItemListGroupBySubOrder, function(groundValue, groundKey) {
                               for (var j = 0; j < groundValue.orderItemList.length; j++) {
                                if (groundValue.orderItemList[j].prepareStatus == "COMPLETE" || groundValue.orderItemList[j].itemDetail != $scope.filterKitcher.filterWord) {
                               }else{
                                   value.parent=true;
    ...
    $scope.filterKitcher.runnerKey=$scope.unique5($scope.filterKitcher.runnerKey);
            ...
            var d = JSON.parse(e.data);
             ...
             if (!!d.orderInfo) {
                    ...
                    if (d.orderInfo.voidOrder == true) {
                        ...
                        angular.forEach($scope.orderList, function(value, key) {
                            //if (voidItemStr.indexOf("*" + value.id) != -1) {
        ...
        console.log(angular.copy($scope.orderList ));
            if (!!$scope.orderList) {
                angular.forEach($scope.orderList, function(value, key) {
                    value.show = false;
                    value.showRecall = false;
                        ...
                        angular.forEach(orderValue.orderItemListGroupBySubOrder, function(groundValue, groundKey) {
                            ...
                            angular.forEach(groundValue.orderItemList, function(setValue, setKey) {
                                ...
                                if (!value.orderStatus) {
                                    console.log("598");
                                    if (setValue.prepareStatus != "COMPLETE") {
                                        $scope.orderList[key].show = true;
                                        $scope.orderList[key].kitchShow = true;
                                    } else if (setValue.prepareStatus == "COMPLETE") {
                                        $scope.orderList[key].kitchRecallShow = true;
                                    }
                                }
    ...
    .controller('RunnerSearchCtrl', function($scope,$location, $rootScope, $timeout, $ionicModal, localStorageService, $http, $timeout) {
  ...
  angular.forEach($scope.orderList, function(value, key) {
   ...
   $scope.done = function(item) {
           ...
           processJson.orderItemId = [];
            ...
            if(item.currentTarget.innerText=="Served"){
            ...
            angular.forEach($scope.orderList, function(value, key) {
                    ...
                    angular.forEach(value.orderItemList, function(orderValue, orderKey) {
                        angular.forEach(orderValue.orderItemListGroupBySubOrder, function(groundValue, groundKey) {
                            angular.forEach(groundValue.orderItemList, function(setValue, setKey) {
                                processJson.orderItemId.push(setValue.id);
